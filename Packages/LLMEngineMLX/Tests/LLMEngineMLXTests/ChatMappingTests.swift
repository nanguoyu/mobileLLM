...
private final class MLXGovernanceProbe: GenerationControlling, @unchecked Sendable {
...
private struct MLXNominalThermal: ThermalGoverning {
...
final class ChatMappingTests: XCTestCase {
    ...
    func testGenerateActuallyEntersInjectedGovernanceBeforeEngineState() async {
        let probe = MLXGovernanceProbe()
        let engine = MLXLLMEngine(governanceFactory: {
            GenerationGovernance(control: probe, thermalGovernor: MLXNominalThermal())
        })
        ...
        } catch let error as MLXLLMEngine.EngineError {
