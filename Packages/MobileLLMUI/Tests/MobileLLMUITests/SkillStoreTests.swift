...
final class SkillStoreTests: XCTestCase {
    ...
    /// Reload from a fresh store at the same URL. Writes are awaited now, so this helper no longer has to
    /// race a fire-and-forget persistence task.
    ...
    func testFirstLoadSeedsBuiltIns() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
    ...
    func testSecondLoadDoesNotDuplicateBuiltIns() async throws {
        try await SkillStore(fileURL: url).load()   // first instance seeds + writes to disk
        let relaunched = SkillStore(fileURL: url)
        try await relaunched.load()                 // fresh instance at the same file
    ...
    func testRepeatedLoadOnSameInstanceIsIdempotent() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
        try await store.load()   // second call is a no-op (hasLoaded guard)
    ...
    func testCreateUpdateDeletePersistAcrossReload() async throws {
        ...
        let created = try await store.create(name: "Pirate", emoji: "🏴", summary: "arr",
        ...
        try await store.update(Skill(id: created.id, name: "Fancy Pirate", emoji: "🏴", summary: "arr",
    ...
    func testBuiltInsCannotBeUpdatedOrDeleted() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
        let builtIn = store.builtInSkills[0]

        try await store.update(Skill(id: builtIn.id, name: "HACKED", emoji: "💀", summary: "x",
    ...
    func testDuplicateMakesEditableCustomCopy() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
        ...
        try await store.update(Skill(id: copy.id, name: "Mine", emoji: copy.emoji, summary: copy.summary,
                                     instructions: "changed", isBuiltIn: false))
        XCTAssertEqual(store.skill(id: copy.id)?.instructions, "changed")
    }

    func testCreateTrimsAndClampsEmojiToOne() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
        let skill = try await store.create(name: "  Spaced  ", emoji: "🔥🔥🔥", summary: "  s  ",
                                           instructions: "  do it  ")
    ...
    func testFailedMutationThrowsAndKeepsPublishedStateDurable() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()
        try FileManager.default.removeItem(at: dir)
        try Data("blocker".utf8).write(to: dir)

        do {
            _ = try await store.create(name: "Ghost", emoji: "👻", summary: "", instructions: "Never land")
            XCTFail("create should throw when the parent path is a regular file")
        } catch {
            // Expected.
        }
        XCTAssertEqual(store.skills, Skill.builtIns,
                       "the observable list changes only after the durable write succeeds")
    }

    func testConcurrentCreatesAreSerializedWithoutLostUpdates() async throws {
        let store = SkillStore(fileURL: url)
        try await store.load()

        async let first = store.create(name: "One", emoji: "1️⃣", summary: "", instructions: "one")
        async let second = store.create(name: "Two", emoji: "2️⃣", summary: "", instructions: "two")
        _ = try await (first, second)

        XCTAssertEqual(Set(store.customSkills.map(\.name)), ["One", "Two"])
        let relaunched = SkillStore(fileURL: url)
        try await relaunched.load()
        XCTAssertEqual(Set(relaunched.customSkills.map(\.name)), ["One", "Two"])
    }
}
